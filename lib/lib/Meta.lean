...
def withFieldTypeAux (s field : Name)
  ...
  OptionT m α := do
let env ← getEnv
let s' ← OptionT.mk <| return findField? env s field
let info ← OptionT.mk <| return getFieldInfo? env s' field
let d ← OptionT.mk <| return env.find? info.projFn
let t := d.type
...
let env ← getEnv
let s' ← OptionT.mk <| return findField? env s field
OptionT.mk <| return getFieldInfo? env s' field
